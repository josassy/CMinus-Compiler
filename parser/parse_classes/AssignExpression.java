...
public class AssignExpression extends Expression {
  ...
  public void genLLCode(Function fun) throws CodeGenerationException {
    ...
    if (fun.getTable().containsKey(ve.id)) {
      ...
      int lhsDest = (int) fun.getTable().get(ve.id);
    ...
    else if (CMinusCompiler.globalHash.containsKey(ve.id)) {
      ...
      Operand localStoreLocation = new Operand(Operand.OperandType.REGISTER, rhsDest);
      storeOperation.setSrcOperand(1, globalStoreLocation);
      storeOperation.setSrcOperand(0, localStoreLocation);

      fun.getCurrBlock().appendOper(storeOperation);
